...
jQuery.fn.extend({
  cropzee: function (options = {
        ...
        modalAnimation: '',
        // cropzee options
        videoExtensions: ['flv', 'avi', 'mov', 'mpg', 'wmv', 'm4v', 'mp3', 'wma', '3gp', 'mp4' ,'FLV' , 'AVI', 'MOV', 'MPG', 'WMV' , 'M4V', 'MP3', 'WMA', '3GP', 'MP4' ], // input extensions supported
        allowedInputs: ['gif', 'jpeg', 'png', 'jpg' , 'PNG' , 'GIF' , 'JPEG', 'JPG', 'flv', 'avi', 'mov', 'mpg', 'wmv', 'm4v', 'mp3', 'wma', '3gp', 'mp4' ,'FLV' , 'AVI', 'MOV', 'MPG', 'WMV' , 'M4V', 'MP3', 'WMA', '3GP', 'MP4'], // input extensions supported
        ...
        if (!options.allowedInputs) {
            options.allowedInputs = ['gif','png','jpg','jpeg', 'mp4'];
        }
        ...
        window.cropzeeCreateImage = function(id) {
            ...
            if (!isEmpty(document.getElementsByClassName('create-image-previewer')[0].width)){

                 var dataURL = document.getElementsByClassName('create-image-previewer')[0].toDataURL();
                 $('#cropped-post-image').val(dataURL);
             }else{
                 var dataURL = document.getElementsByClassName('edit-image-previewer')[0].toDataURL();
                 $('#edit-cropped-post-image').val(dataURL);
        ...
        function cropzeeTriggerModal(id, src) {
            ...
            '<div class="light-modal" id="cropzee-modal" role="dialog" aria-labelledby="light-modal-label" aria-hidden="false" data-lightmodal="close">'
                + '<div class="light-modal-content ' + animation + '">'
                    ...
                    + '<div class="light-modal-footer" style="justify-content: space-between;">'
                        + '<div onclick="closeModal()" class="light-modal-close-btn" style="cursor: pointer;" aria-label="close" style ="background: #757373!important;">Cancel</div>'
                        + '<div onclick="cropzeeRotateImage(`' + id + '`);" class="btn btn-primary" style="cursor: pointer;" style ="background: #757373!important;">Rotate 90deg</div>'
                        + '<div onclick="cropzeeCreateImage(`' + id + '`);" class="btn btn-success" style="cursor: pointer;" style ="background: #757373!important; ">Done</div>'
                    + '</div>'
                + '</div>'
            ...
            setTimeout(function(){
                ...
                setTimeout(function(){
                    ...
                    if(window.localStorage.getItem("update_post_pic") == "update-post-pic"){

                        $('#update-a-post-model').modal('hide');
                    }else if (window.localStorage.getItem("create_post_pic") == "create-post-pic")
                    {
                    ...
                    $('.light-modal').css({ 'visibility' : 'visible', 'justify-content': 'center' ,     'background': 'rgba(0,0,0,.5)' });
        ...
        $(this).click(function(){
            ...
            $(this).one("change", function(){
                ...
                if($.inArray(ext, options.allowedInputs) == -1) {
                    ...
                    if (['flv', 'avi', 'mov', 'mpg', 'wmv', 'm4v', 'mp3', 'wma', '3gp', 'mp4' ,'FLV' , 'AVI', 'MOV', 'MPG', 'WMV' , 'M4V', 'MP3', 'WMA', '3GP', 'MP4' ].includes(ext)) {
...
function closeModal() {
    ...
    if(window.localStorage.getItem("update_post_pic") == "update-post-pic"){
        ...
        $('#update-a-post-model').modal('show');
    }else if (window.localStorage.getItem("create_post_pic") == "create-post-pic")
    {
        $('#postModal').modal('show');

    }
    localStorage.removeItem("create_post_pic");
    localStorage.removeItem("update_post_pic");
    $('.light-modal').css({ 'visibility' : 'hidden'});
